...
with tf.device('/GPU:0'):
    efficientnetB0_model = tf.keras.models.load_model('/home/axernox/Project_2/efficientnetb0.keras', compile=False)
    unet_model = tf.keras.models.load_model('/home/axernox/Project_2/Lung_Segmentation/lung_segmentation.keras', compile=False)
...
def lung_segmentation(image):
    ...
    cropped_xray = cv2.bitwise_and(xray_resized, xray_resized, mask=binary_mask)

    y_indices, x_indices = np.where(binary_mask > 0)
    x_min, x_max = x_indices.min(), x_indices.max()
    y_min, y_max = y_indices.min(), y_indices.max()

    final_cropped_xray = cropped_xray[y_min:y_max, x_min:x_max]

    return final_cropped_xray
...
def get_prediction(image):
    pred = efficientnetB0_model.predict(image)
    print('***********')
    print(pred)
...
st.markdown(
    f"<p style='text-align:justify; color:#000000; font-size:20px'>This WebApp leverages the power of deep learning to assist in the early detection of Tuberculosis (TB) from chest X-ray images. Built with a trained EfficientNetB0 model, the app classifies uploaded chest X-rays into Drug-Sensitive TB and Drug-Resistant TB.<br>\
        It showcases the image processing which we do on the uploaded image. Image processing includes applying the filters like CLAHE, Gabor, Canny-Edge and on top of CLAHE, how we are doing the Lung Segmentation and Lung Cropping.\
...
if img_upload is not None:
    ...
    if st.button("Predict"):
        ...
        lung_segment = lung_segmentation(img_clahe)
        ...
        st.text("")
        lung_col1, lung_col2 = st.columns(2)
        with lung_col1:
            st.image(lung_segment, caption="Lung Segmentation", use_container_width=True)
